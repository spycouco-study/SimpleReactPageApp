...
const BoundingBoxEditor = () => {
  ...
  const handleMouseMove = (e) => {
    ...
    if (isResizing && selectedBox !== null && resizeHandle) {
      ...
      setBoxes((prev) =>
        prev.map((box, i) => (i === selectedBox ? updatedBox : box))
      );
      drawCanvas();
      return;
    }
  ...
  useEffect(() => {
    if (image && previewCanvasRef.current && boxes.length > 0) {
      ...
      const sourceX =
        (currentBox.width > 0
          ? currentBox.startX
          : currentBox.startX + currentBox.width) * scaleX;
      const sourceY =
        (currentBox.height > 0
          ? currentBox.startY
          : currentBox.startY + currentBox.height) * scaleY;
      ...
      ctx.drawImage(
        image,
        sourceX,
        sourceY, // 원본 이미지에서 잘라낼 영역의 시작점
        boxWidth,
        boxHeight, // 원본 이미지에서 잘라낼 영역의 크기
        0,
        0, // 캔버스에 그릴 위치
        displayBoxWidth,
        displayBoxHeight // 캔버스에 그릴 크기
      );
    }
    ...
    <div className="bounding-box-editor">
      ...
      <div className="preview-section">
        ...
        <div className="controls">
          ...
          <span>
            프레임: {currentFrame + 1} / {boxes.length}
          </span>
        </div>
      </div>
    </div>
